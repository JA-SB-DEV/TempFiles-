...
import { Hourglass, ShieldCheck, Copy, CheckCircle2, QrCode, ArrowLeft, Zap, Github, Repeat, Link as LinkIcon, Eye, ExternalLink, Infinity, Sun, Moon, Activity, BarChart3, Info, Terminal, Menu, X, MoreVertical } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    const handleOrientation = (event: DeviceOrientationEvent) => {
        const beta = event.beta;  // X-axis tilt (-180 to 180)
        const gamma = event.gamma; // Y-axis tilt (-90 to 90)
        
        if (beta !== null && gamma !== null) {
            // Constrain movement and invert slightly for depth feel
            const x = Math.min(Math.max(gamma, -45), 45) * 1.5; 
            const y = Math.min(Math.max(beta, -45), 45) * 1.5;
  ...
  const UtilityButtons = () => (
      <>
        <button 
            ...
            className="p-2.5 rounded-full bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 shadow-md border border-slate-200 dark:border-slate-700 hover:scale-105 transition-transform hover:text-blue-500 dark:hover:text-blue-400 w-full md:w-auto flex md:inline-flex items-center justify-center gap-2"
            title="Información y Ayuda"
        >
            <Info size={18} /> <span className="md:hidden text-sm font-bold">Ayuda</span>
        </button>

        <button 
            onClick={() => { setShowStats(true); setIsMobileMenuOpen(false); }}
            className="p-2.5 rounded-full bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 shadow-md border border-slate-200 dark:border-slate-700 hover:scale-105 transition-transform hover:text-violet-500 dark:hover:text-violet-400 w-full md:w-auto flex md:inline-flex items-center justify-center gap-2"
            title="Estadísticas Globales"
        >
            <BarChart3 size={18} /> <span className="md:hidden text-sm font-bold">Estadísticas</span>
        </button>

        <button 
            onClick={() => { setShowAudit(true); setIsMobileMenuOpen(false); }}
            className="p-2.5 rounded-full bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 shadow-md border border-slate-200 dark:border-slate-700 hover:scale-105 transition-transform hover:text-cyan-500 dark:hover:text-cyan-400 w-full md:w-auto flex md:inline-flex items-center justify-center gap-2"
            title="Auditoría del Sistema"
        >
            <Activity size={18} /> <span className="md:hidden text-sm font-bold">Sistema</span>
        </button>
        
        <button 
            onClick={() => { toggleRetro(); setIsMobileMenuOpen(false); }}
            className={`p-2.5 rounded-full shadow-md border hover:scale-105 transition-all w-full md:w-auto flex md:inline-flex items-center justify-center gap-2 ${isRetro ? 'bg-black border-green-500 text-green-500' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700'}`}
            title="Modo Terminal Retro (Alto Contraste)"
        >
            <Terminal size={18} /> <span className="md:hidden text-sm font-bold">Modo Retro</span>
        </button>
      </>
  );

  return (
    <div className="min-h-screen relative overflow-hidden text-slate-800 dark:text-slate-200 font-sans selection:bg-cyan-500/30 cursor-none md:cursor-default transition-colors">
      ...
      {/* --- PARALLAX WRAPPER --- */}
      <div 
        className="fixed inset-0 pointer-events-none transition-transform duration-100 ease-out z-0"
        style={{ transform: `translate(${parallaxOffset.x}px, ${parallaxOffset.y}px)` }}
      >
        <div className="absolute top-0 left-1/4 w-96 h-96 bg-cyan-500/10 rounded-full blur-[100px] animate-blob mix-blend-multiply dark:mix-blend-screen"></div>
        <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-violet-500/10 rounded-full blur-[100px] animate-blob animation-delay-2000 mix-blend-multiply dark:mix-blend-screen"></div>
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        {/* Responsive Header */}
        <header className="p-4 md:p-6 w-full max-w-5xl mx-auto">
            <div className="flex flex-col gap-4">
                
                {/* Top Row: Logo, Theme, Mobile Menu */}
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 cursor-pointer group" onClick={resetApp}>
                        <div className="relative">
                            ...
                            <Hourglass className="text-cyan-600 dark:text-cyan-400 relative z-10 transition-transform group-hover:rotate-180 duration-700" size={28} />
                        </div>
                        <div>
                            <h1 className="text-xl md:text-2xl font-black tracking-tighter text-slate-900 dark:text-white">
                                CHRONOS
                            </h1>
                            <p className="text-[9px] md:text-[10px] text-cyan-600 dark:text-cyan-400/80 font-mono tracking-widest uppercase">Bóveda Efímera</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="md:hidden relative">
                             <button 
                                ...
                                className={`p-2.5 rounded-full shadow-md border transition-colors ${isMobileMenuOpen ? 'bg-cyan-500 text-white border-cyan-500' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700'}`}
                             >
                             ...
                             {isMobileMenuOpen && (
                                 <div className="absolute top-full right-0 mt-2 w-48 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl p-2 flex flex-col gap-2 animate-fade-in z-50">
                                     <UtilityButtons />
                                 </div>
                             )}
                        </div>
                ...
                {/* Second Row: Navigation Toggle (Always visible but full width on mobile) */}
                {!uploadedFile && (
                    <div className="flex justify-center w-full">
                        <nav className="flex p-1 bg-white/50 dark:bg-slate-900/60 rounded-full border border-slate-200 dark:border-slate-700/50 backdrop-blur-md shadow-lg w-full md:w-auto justify-between md:justify-start">
                            <button 
                                onClick={() => setView('upload')}
                                className={`flex-1 md:flex-none px-6 py-2 rounded-full text-sm font-bold transition-all duration-300 ${view === 'upload' ? 'bg-cyan-500 text-white dark:text-slate-950 shadow-[0_0_20px_rgba(6,182,212,0.3)]' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5'}`}
                            >
                                Subir
                            </button>
                            <button 
                                onClick={() => setView('retrieve')}
                                className={`flex-1 md:flex-none px-6 py-2 rounded-full text-sm font-bold transition-all duration-300 ${view === 'retrieve' ? 'bg-emerald-500 text-white dark:text-slate-950 shadow-[0_0_20px_rgba(16,185,129,0.3)]' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5'}`}
                            >
                                Recuperar
                            </button>
                        </nav>
                ...
                <div className="w-full max-w-md animate-fade-in perspective-1000">
                    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-700/50 rounded-3xl p-8 text-center shadow-2xl relative overflow-hidden group">
                        ...
                        <div className="bg-slate-100 dark:bg-black/40 rounded-2xl p-6 mb-8 border border-slate-200 dark:border-slate-700/50 relative overflow-hidden">
                            ...
                            <div className="text-5xl font-mono font-bold text-slate-900 dark:text-white tracking-wider mb-4 select-all drop-shadow-[0_0_10px_rgba(34,211,238,0.5)] relative z-10 break-words">
                                {uploadedFile.code}
                            </div>
                            
                            <div className="flex gap-2 relative z-10 flex-col md:flex-row">
                                <button 
                                ...
                                >
                                    ...
                                    {linkCopySuccess ? "Copiado" : "Copiar Link"}
                                </button>
                            </div>
                        </div>
